	...
	private void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("GET".equals(method)) {
			...
			if (dao.login(manager)) {
				...
				// login 시에는 HTTP 세션을 얻어올 때 true 로 얻어내야 함
...
//			bookMap.put("bookSeq", bookSeq);
			...
			try {
				...
				// 입력 성공 메시지 생성
				String message = String.format("도서 정보 1건 [%s] 추가 성공", title);
				request.setAttribute("message", message);
	...
	private void delete(HttpServletRequest request
		...
		try {
			...
			// (1) 삭제 성공 화면 이동
			// 삭제 성공 메시지 생성
			String message = String.format("도서 정보[일련번호:%s] 삭제 성공", book.getBookSeq());
			request.setAttribute("message", message);
		...
		// 삭제 성공 메시지를 2초간 보여준후 이동할 nextPage 설정
		// 삭제 후에는 보여줄 상세페이지가 없으므로 목록으로 이동
		String nextPage = "/main?action=select";
		request.setAttribute("nextPage", nextPage);
		
		// 메인 컨텐트 화면에 강라끼울 content 속성 설정
		String mainContent = "/messageBook";
		request.setAttribute("content", mainContent);
	...
	private void update(HttpServletRequest request
		...
		if ("GET".equals(method)) {
			// 2. GET 으로 진입하는 경우 수정을 위한 화면 제공
			// (1) 요청으로 전달된 파라미터 bookSeq 추출
			int bookSeq = Integer.valueOf(request.getParameter("bookSeq"));
			...
			try {
				// (4) update 쿼리 실행
				dao.update(bookMap); // 수정하는 건 이 한 줄이면 가능
				...
				// 수정 실패 메시지 출력 후 이동할 nextPage 설정 : 
				// 목록으로 이동
				String nextPage = "main?action=select";
				request.setAttribute("nextPage", nextPage);
	 ...
	 * @throws ServletException 
	 */
	private void datail(HttpServletRequest request
					  , HttpServletResponse response) throws ServletException, IOException {
